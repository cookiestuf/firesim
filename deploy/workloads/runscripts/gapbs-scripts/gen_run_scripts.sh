...
root_fs="gapbs.img"
ini_file="ref-gapbs.ini"

#more default values
SUITE="bc bfs cc cc_sv pr sssp tc ../gapbs.sh"
workload_file="ref-gapbs.json"
...
do
    case "$1" in
         ...
         --input)
             shift;
             input_type=$1
             workload_file="$input_type-gapbs.json"
             ini_file="$input_type-gapbs.ini"
             command_file=$1.cmd
             ;;
         -h)
             usage
...
then
    ...
    if [ "$input_type" = ref ]; then
        aws s3 cp s3://gapbs-ref-graphs --recursive
    else
